...
async def _enrich_with_depth(img_data: List[Dict], collector: Any, target_usdt: float) -> None:
    ...
    try:
        # Check if collector has the method (it might be missing in test mocks)
        if hasattr(collector, 'depth_liquidity_collector') and collector.depth_liquidity_collector:
             # Update the collector's watchlist if needed (optional optimization)
             pass
    except Exception:
        pass
    ...
    for row in img_data:
        try:
            ...
            if hasattr(collector, 'get_depth_liquidity_range'):
                # This calls the updated method in DataCollector
                rng = collector.get_depth_liquidity_range(base, target_usdt, mid_price=mid_hint)
                
                # Retry once if needed (sometimes helps with race conditions)
                if rng is None:
            ...
            row.update({
                ...
                "depth_best_bid": rng.get("mid"), # Using mid as proxy
